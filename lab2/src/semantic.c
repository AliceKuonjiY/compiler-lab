...
void semantic_analysis(Node root) {
    ...
    symbolTable = new_stack();
    ExtDefList(getChild(root, 0));
    free_stack(&symbolTable);
}
...
static Type Specifier(Node node) {
    ...
    if (strcmp(child->nodeName, "TYPE") == 0) { // 基本类型
        Type type = malloc(sizeof(struct Type_));
        type->kind = BASIC;
        if (child->valType == TYPE_INT) {
            type->info.basic = INT;
        } else {
            type->info.basic = FLOAT;
        }
        return type;
    } else { // 结构体类型
...
static Type StructSpecifier(Node node) {
    ...
    if (strcmp(tag->nodeName, "OptTag") == 0) { // 命名结构体定义
        ...
        pop_stack(&symbolTable);
        if (find_in_stack(symbolTable, type->info.structure.name) != NULL) {
            // !ERROR
            printError(16, node->lineNum, "Redefined structure.");
            return NULL;
        }
        insert_in_bottom(symbolTable, type->info.structure.name, type); // ?HACK
        return type;
        ...
        Node id = getChild(tag, 0);
        List found = find_in_stack(symbolTable, id->valStr);
        if (found == NULL) {
            // !ERROR
...
static void VarDec(Node node, Type type, List *head, int structFlag) {
    ...
    if (strcmp(first->nodeName, "ID") == 0) { // 变量声明
        List found = find_in_top(symbolTable, first->valStr);
        if (found != NULL) {
            // !ERROR
            if (structFlag) {
        ...
        insert_in_top(symbolTable, first->valStr, type);
...
static Type Exp(Node node) {
    ...
    if (strcmp(first->nodeName, "ID") == 0) { // 变量或函数调用
        List found = find_in_stack(symbolTable, first->valStr);
        if (getChild(node, 1) == NULL) { // 变量
            if (found == NULL ||
                found->type == NULL ||
                found->type->kind == FUNCTION ||
                (found->type->kind == STRUCTURE &&
            ...
            if (strcmp(getChild(node, 2)->nodeName, "Args") == 0) {
                ...
                *use = *found->type;
                ...
                if (!check_in_stack(symbolTable, use->info.function.name, use)) {
                    // !ERROR
        ...
        *type = *expType;
        ...
        if (strcmp(second->nodeName, "ASSIGNOP") == 0) {
            ...
            *type = *exp1;
            ...
            if (exp2->kind != BASIC || exp2->info.basic != INT) {
                // !ERROR
