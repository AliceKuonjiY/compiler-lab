...
static int field_check(TypeList p1, TypeList p2);
...
TypeList new_list(char *name, Type type) {
    TypeList list = (TypeList)malloc(sizeof(struct TypeList_));
    list->name = name;
    list->type = type;
    list->next = NULL;
    return list;
...
void insert_list(TypeList *head, char *name, Type type) {
    TypeList newnode = new_list(name, type);
    newnode->next = *head;
    *head = newnode;
}
...
int calc_size(Type type) {
    ...
    if (type->kind == TYPE_BASIC) {
        ...
        int size = 0;
        TypeList field = type->info.structure.domain;
        while (field != NULL) {
