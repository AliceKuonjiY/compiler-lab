...
TypeStack check_symbolTable;
int errorLine[5000] = {0};

static void printError(int type, int line, const char *msg);
static void ExtDefList(ASTNode node);
static void ExtDef(ASTNode node);
static void ExtDecList(ASTNode node, Type type, TypeList *head);
static Type FunDec(ASTNode node, Type returnType);
static void CompSt(ASTNode node, Type returnType);
static void StmtList(ASTNode node, Type returnType);
static void Stmt(ASTNode node, Type returnType);
static void VarList(ASTNode node, TypeList *head);
static void ParamDec(ASTNode node, TypeList *head);
static Type Specifier(ASTNode node);
static Type StructSpecifier(ASTNode node);
static void DefList(ASTNode node, TypeList *head, int structFlag);
static void Def(ASTNode node, TypeList *head, int structFlag);
static void DecList(ASTNode node, Type type, TypeList *head, int structFlag);
static void Dec(ASTNode node, Type type, TypeList *head, int structFlag);
static void VarDec(ASTNode node, Type type, TypeList *head, int structFlag);
static Type Exp(ASTNode node);
static void Args(ASTNode node, TypeList *head);
...
void semantic_analysis(ASTNode root) {
    ...
    *(write_->info.function.type) = (struct Type_){ .kind = TYPE_BASIC, .info.basic = BASIC_INT, .assign = BOTH };
    write_->info.function.param = malloc(sizeof(struct TypeList_));
...
static Type StructSpecifier(ASTNode node) {
    ...
    if (strcmp(tag->nodeName, "OptTag") == 0) { // 命名结构体定义
        ...
        ASTNode id = getChild(tag, 0);
        TypeList found = find_in_stack(check_symbolTable, id->valStr);
        if (found == NULL) {
            // !ERROR
...
static void VarDec(ASTNode node, Type type, TypeList *head, int structFlag) {
    ...
    if (strcmp(first->nodeName, "ID") == 0) { // 变量声明
        TypeList found = find_in_top(check_symbolTable, first->valStr);
        if (found != NULL) {
            // !ERROR
            if (structFlag) {
...
static Type Exp(ASTNode node) {
    ...
    if (strcmp(first->nodeName, "ID") == 0) { // 变量或函数调用
        TypeList found = find_in_stack(check_symbolTable, first->valStr);
