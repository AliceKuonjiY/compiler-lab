...
static CPValue meetValue(CPValue v1, CPValue v2) {
    if (v1.kind == UNDEF && v2.kind == UNDEF) {
        return get_UNDEF(); // 两个都是 UNDEF, 返回 UNDEF
    } else if (v1.kind == UNDEF) {
        return v2; // 一个是 UNDEF, 返回另一个
    } else if (v2.kind == UNDEF) {
        return v1; // 一个是 UNDEF, 返回另一个
    } else if (v1.kind == CONST && v2.kind == CONST) {
        if (v1.const_val == v2.const_val) {
            return v1; // 两个都是相同的 CONST, 返回该 CONST
        } else {
            return get_NAC(); // 两个都是不同的 CONST, 返回 NAC
        }
    } else {
        return get_NAC(); // 其他情况, 返回 NAC
    }
}

static CPValue calculateValue(IR_OP_TYPE IR_op_type, CPValue v1, CPValue v2) {
    if(v1.kind == CONST && v2.kind == CONST) {
        int v1_val = v1.const_val, v2_val = v2.const_val;
        int res_const;
        switch (IR_op_type) {
            case IR_OP_ADD:
                res_const = v1_val + v2_val;
                break;
            case IR_OP_SUB:
                res_const = v1_val - v2_val;
                break;
