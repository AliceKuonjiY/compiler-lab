...
static InterCode ExtDefList(ASTNode node);
static InterCode ExtDef(ASTNode node);
static InterCode ExtDecList(ASTNode node, Type type);
static InterCode FunDec(ASTNode node);
...
static InterCode FunDec(ASTNode node) {
    InterCode code1 = new_labelop(LABELOP_FUNCTION, (IRLabel){ LABEL_FUNCTION, .name = getChild(node, 0)->valStr, -1 });
